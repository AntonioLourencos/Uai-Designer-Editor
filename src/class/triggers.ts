...
import { ElementD } from "../store/elements";
...
class Trigger extends Environment {
  ...
  listenEvents() {
    window.addEventListener("pinia-create-element", (event) => {
      const { detail } = event as CustomEvent<{
        index: number;
        element: ElementD;
      }>;

      this.insertShape({ ...detail });
    });
  }
  ...
  shapeSelection() {
    ...
    useFocusStore().$subscribe((mutation, value) => {
      ...
      if (focusedEl.id != "environment") {
        const isCanvas =
          focusedEl.classList.contains("konvajs-content") ||
          focusedEl.localName == "canvas";
        if (!isCanvas) {
          stopSelect();
        }
      }
